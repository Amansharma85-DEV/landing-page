...
const seedDatabase = async () => {
  try {
    ...
    await new Theme({
      primaryColor: '#c5a880', // Gold/Champagne
      secondaryColor: '#161616', // Jet Black
      accentColor: '#ffffff',
      backgroundColor: '#0c0c0c', // Pure Dark
      buttonStyle: 'rounded',
    ...
    await new Setting({
      name: 'L’Étoile Dorée',
      logo: '', // empty to use letters or text-logo initially
    ...
    const catAppetizer = await new Category({ name: 'Appetizers', slug: 'appetizers' }).save();
    const catMain = await new Category({ name: 'Mains', slug: 'mains' }).save();
    const catDessert = await new Category({ name: 'Desserts', slug: 'desserts' }).save();
    const catWine = await new Category({ name: 'Fine Wines', slug: 'wines' }).save();
